  ...
  while (std::getline(m_objFile, current))
  {
    if (current.size() > 0)
    {
      ...
      if (current.at(0) == 'v' && current.at(1) == ' ')
      {
        ...
        {
          if (mainCounter > 0)
            m_vertices.push_back(std::stof(tmp));
          mainCounter++;
        }
      }
      // TEXTURE COORD
